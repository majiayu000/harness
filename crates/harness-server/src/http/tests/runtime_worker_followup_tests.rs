...
#[tokio::test]
async fn runtime_job_worker_auto_submits_prompt_task_child_workflow() -> anyhow::Result<()> {
    if !crate::test_helpers::db_tests_enabled().await {
        return Ok(());
    }
...
#[tokio::test]
async fn runtime_job_worker_auto_submits_prompt_task_child_with_dependencies() -> anyhow::Result<()>
{
    if !crate::test_helpers::db_tests_enabled().await {
        return Ok(());
    }
    ...
    assert_eq!(
        ...
        "prompt-task:owner/repo:issue:127"
    );
    let child_commands = store.commands_for(&child_id).await?;
    assert!(
