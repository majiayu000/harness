...
#[tokio::test]
async fn runtime_approval_route_rejects_stale_request_for_single_aliased_turn() -> anyhow::Result<()>
{
    let dir = tempfile::tempdir()?;
    let thread_manager = crate::thread_manager::ThreadManager::new();
    let thread_id = thread_manager.start_thread(dir.path().to_path_buf());
    ...
    thread_manager.add_item(
        ...
        &turn_id,
        harness_core::types::Item::ApprovalRequest {
            id: Some("request-current".to_string()),
            action: "run tests".to_string(),
            approved: None,
        },
    )?;
    ...
    assert!(!called.load(Ordering::SeqCst));
    assert_eq!(
        thread_manager.pending_approval_items_for_runtime_handle("submission-1"),
        vec![harness_core::types::Item::ApprovalRequest {
            id: Some("request-current".to_string()),
            action: "run tests".to_string(),
            approved: None,
        }]
