...
#[tokio::test]
async fn password_reset_empty_email_returns_400() -> anyhow::Result<()> {
    let dir = tempfile::tempdir()?;
    let state =
        make_test_state_with_config(dir.path(), harness_core::config::HarnessConfig::default())
            .await?;
    ...
    assert!(
        ...
        "expected 'email is required' in error body, got: {json}"
    );
    Ok(())
}

#[tokio::test]
async fn password_reset_rate_limit_returns_429() -> anyhow::Result<()> {
    let dir = tempfile::tempdir()?;
    let mut config = harness_core::config::HarnessConfig::default();
    config.server.password_reset_rate_limit_per_hour = 2;
    let state = make_test_state_with_config(dir.path(), config).await?;

    let body = serde_json::json!({"email": "user@example.com"}).to_string();

    for _ in 0..2 {
        let app = password_reset_app(state.clone());
        let _ = app
            .oneshot(
                Request::builder()
                    .method("POST")
