...
fn parse_issues_opened_with_harness_mention_creates_issue_task() {
    ...
    let (request, reason) =
        parse_github_webhook_task_request("issues", payload.to_string().as_bytes(), false, None)
            .unwrap();
    let request = request.expect("request should exist");
...
fn ping_event_returns_none_request() {
    let (request, reason) =
        parse_github_webhook_task_request("ping", br#"{"zen":"anything"}"#, false, None).unwrap();
    assert!(request.is_none());
...
fn pull_request_review_invalid_payload_returns_error() {
    let result =
        parse_github_webhook_task_request("pull_request_review", b"not valid json", false, None);
    assert!(result.is_err());
    assert!(result
        .unwrap_err()
