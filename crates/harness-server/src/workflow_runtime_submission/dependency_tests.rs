...
async fn issue_submission_releases_dependency_on_completed_runtime_handle() -> anyhow::Result<()> {
    ...
    crate::test_helpers::force_upsert_runtime_instance_for_test(&store, &dep_workflow).await?;
...
async fn issue_submission_keeps_blocked_runtime_dependency_without_closed_evidence_waiting(
    ...
    let waiting = release_ready_issue_dependencies(&store, 10).await?;
...
async fn dependency_release_rotates_waiting_rows_to_prevent_starvation() -> anyhow::Result<()> {
    ...
    let ready_dep = record_issue_submission(
        &store,
        IssueSubmissionRuntimeContext {
            project_root: &project_root,
            repo: Some("owner/repo"),
            issue_number: 79,
            task_id: &ready_dep_id,
            labels: &labels,
            force_execute: false,
            additional_prompt: None,
            depends_on: &[],
    ...
    .await?;
    let mut ready_dep_workflow = store
        .get_instance(&ready_dep.workflow_id)
        .await?
        .expect("ready dependency workflow should exist");
    ready_dep_workflow.state = "done".to_string();
    crate::test_helpers::force_upsert_runtime_instance_for_test(&store, &ready_dep_workflow)
        .await?;
    ...
    let first = release_ready_issue_dependencies(&store, 1).await?;
    assert_eq!(first.waiting, 1);
    assert_eq!(first.released, 0);
    let second = release_ready_issue_dependencies(&store, 1).await?;
