...
pub async fn build_app_state(server: Arc<HarnessServer>) -> anyhow::Result<AppState> {
    ...
    let intake = crate::startup::init_intake_and_services(
        &server,
        &bootstrap,
        &stores,
        &engines,
    ...
    .await?;
    crate::startup::spawn_background_tasks(&stores.tasks, intake.completion_callback.clone());
    let runtime = crate::startup::restore_runtime_state(&bootstrap).await?;
    Ok(crate::startup::assemble_app_state(
        server,
        bootstrap,
        stores,
        engines,
...
pub async fn serve(server: Arc<HarnessServer>, addr: SocketAddr) -> anyhow::Result<()> {
    ...
    crate::intake::build_orchestrator(
        ...
        Some(&crate::startup::builders::expand_tilde(
