...
pub(crate) use pipeline::run_task;

#[cfg(test)]
use harness_core::config::agents::CapabilityProfile;

// Items brought into scope for the test module via `use super::*`.
#[cfg(test)]
use helpers::truncate_validation_error;
#[cfg(test)]
use lifecycle::{
    compute_backoff_ms, jaccard_word_similarity, normalize_issues, parse_implementation_outcome,
    ImplementationOutcome,
};
#[cfg(test)]
use pipeline::prepend_constitution;
#[cfg(test)]
use pipeline::restricted_tools;
#[cfg(test)]
use pr_detection::{
