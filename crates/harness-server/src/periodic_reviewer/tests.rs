use super::tick_helpers::{
    build_fix_prompt, format_violations_for_prompt, pick_secondary_review_agent, poll_task_output,
...
fn test_action_truncated_at_500() {
    ...
    let value = extract_field_from_prompt(&prompt, "Action").expect("Action line must be present");
    assert_eq!(
        value.chars().count(),
        500,
