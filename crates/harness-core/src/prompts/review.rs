...
pub fn review_prompt(
    ...
    format!(
        "{context}\
         ...
         2b. For any review where the author login contains 'gemini' and the state is 'COMMENTED': \
         record the review id and body for later cross-referencing.\n\
         3. Run `gh api repos/{repo}/pulls/{pr}/comments` to read inline review comments; \
         group results by pull_request_review_id.\n\
         3b. Revisit each Gemini COMMENTED review from step 2b whose review id does not appear \
         in any inline comment's pull_request_review_id field (i.e. zero linked inline comments): \
         if the body contains (case-insensitive) any of these phrases anywhere in a line: \
