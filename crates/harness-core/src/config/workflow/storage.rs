...
pub struct WorkflowStoragePolicy {
    ...
    /// Enable workflow-runtime stuck-instance reporting. Enabled by default;
    /// alerts fire on the transition edge, so enabling is not noisy.
    #[serde(default = "default_true")]
    ...
    /// Enable pruning of terminal workflow-runtime history. Enabled by
    /// default; the first passes run in dry-run mode to bound the blast
    /// radius of activation on existing deployments.
    #[serde(default = "default_true")]
    pub runtime_retention_enabled: bool,
    /// Number of retention passes that report what would be deleted without
    /// deleting, counted from process start. `0` disables dry-run.
    #[serde(default = "default_runtime_retention_dry_run_passes")]
    pub runtime_retention_dry_run_passes: u32,
    ...
    /// Enable pruning of terminal task rows and task-owned child rows. Enabled
    /// by default; the first passes run in dry-run mode.
    #[serde(default = "default_true")]
    pub task_retention_enabled: bool,
    /// Number of task-retention passes that report what would be deleted
    /// without deleting, counted from process start. `0` disables dry-run.
    #[serde(default = "default_task_retention_dry_run_passes")]
    pub task_retention_dry_run_passes: u32,
...
impl Default for WorkflowStoragePolicy {
    fn default() -> Self {
        Self {
            ...
            runtime_retention_dry_run_passes: default_runtime_retention_dry_run_passes(),
            ...
            task_retention_dry_run_passes: default_task_retention_dry_run_passes(),
...
fn default_runtime_retention_dry_run_passes() -> u32 {
    1
}
...
fn default_task_retention_dry_run_passes() -> u32 {
    1
}
