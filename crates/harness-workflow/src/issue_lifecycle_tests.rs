...
#[test]
fn pr_detected_rebinds_stage_task_before_pr_open() {
    for state in [
        ...
        let mut workflow = workflow_in(state);
        workflow.active_task_id = Some("previous-stage-task".into());

        workflow
            .apply_event(
                matching_event(IssueLifecycleEventKind::PrDetected).with_task_id("pr-stage-task"),
            )
            .unwrap();

        assert_eq!(workflow.state, IssueLifecycleState::PrOpen);
        assert_eq!(workflow.active_task_id.as_deref(), Some("pr-stage-task"));
        assert_eq!(workflow.pr_number, Some(1));
    }
}

#[test]
fn repeated_pr_detected_requires_compatible_task_and_pr_bindings() {
    let mut workflow = workflow_in(IssueLifecycleState::PrOpen);

    assert_binding_conflict_preserves(
        &mut workflow,
        matching_event(IssueLifecycleEventKind::PrDetected).with_task_id("other-task"),
    );
    assert_binding_conflict_preserves(
        &mut workflow,
        matching_event(IssueLifecycleEventKind::PrDetected).with_pr(2, "other-pr-url"),
    );
}
...
fn metadata_policy(source: IssueLifecycleState, kind: IssueLifecycleEventKind) -> MetadataPolicy {
...
fn required_mutated_fields(
    ...
    use MetadataPolicy as P;
    let mut fields =
        BTreeSet::from(["last_event", "updated_at", "last_remote_fact_hash"].map(str::to_string));
