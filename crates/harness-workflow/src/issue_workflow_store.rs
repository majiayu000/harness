...
impl IssueWorkflowStore {
    ...
    /// Patch only the `project_id` field inside the JSONB blob without replacing the whole row.
    ///
    /// This preserves the historical behavior used by the feedback sweeper when
    /// it can resolve a corrupt worktree path to the canonical project root.
    ...
    /// List workflow rows whose `project_id` still points at a worktree path.
    ...
    /// Replace a workflow row's `project_id` and rekey its primary key.
    ...
    /// Mark a workflow row as `Failed` with a reason.
