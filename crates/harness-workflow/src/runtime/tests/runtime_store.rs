...
async fn runtime_turn_reservation_is_atomic_and_replay_safe() -> anyhow::Result<()> {
    ...
    store
        .upsert_instance(&issue_instance("implementing").with_id(workflow_id))
        .await?;
    ...
    let replay = store
        ...
        .await?;
    assert_eq!(
        first.as_ref().map(|event| &event.id),
        replay.as_ref().map(|event| &event.id)
    );

    let left = store.reserve_runtime_turn_started_for_workflow(
        workflow_id,
        &jobs[0].id,
        3,
        json!({"reservation_key": "retry-race-1"}),
    );
    let right = store.reserve_runtime_turn_started_for_workflow(
        workflow_id,
        &jobs[1].id,
        3,
        json!({"reservation_key": "retry-race-2"}),
    );
    ...
    assert_eq!(
        store
            .runtime_turns_started_for_workflow(workflow_id, None)
            .await?,
        3
    );
    Ok(())
