...
fn candidate_fanout_budget_splits_runtime_profile_max_turns() -> anyhow::Result<()> {
    ...
    let payload = json!({
        "candidate": {"candidate_count": 3, "budget": {"max_turns_per_candidate": null}},
    });
...
fn candidate_fanout_budget_override_wins_over_split() -> anyhow::Result<()> {
    ...
    profile.max_turns = Some(9);
    let payload = json!({
        "candidate": {"candidate_count": 3, "budget": {"max_turns_per_candidate": 5}},
    });
...
fn eval_command(isolation: serde_json::Value) -> WorkflowCommandRecord {
    command_record(WorkflowCommand::new(
        WorkflowCommandType::EnqueueActivity,
        "eval-implement",
        json!({"activity": "implement_issue", "eval": {"timeout_secs": 1800, "isolation": isolation}}),
    ))
}

#[test]
fn eval_isolation_command_policy_overrides_host_defaults() -> anyhow::Result<()> {
    let command = eval_command(json!({
        "tier": "container",
...
fn eval_isolation_command_policy_rejects_host_tier() {
    let command = eval_command(json!({"tier": "host"}));
