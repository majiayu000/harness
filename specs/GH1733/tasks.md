...
- [ ] `SP1733-T4` — Owner: verification and handoff owner. Dependencies: SP1733-T1 through SP1733-T3. Covers: B-001 through B-016. Done when: formatting, focused suites, both package suites, workspace check, workspace clippy, file-size limits, changed-file manifest, public-constructor and production-call-site audits, independent review, every current PR #1862 thread and every valid PR #1859 finding are re-evaluated on the current head, parser tests do not claim source-byte provenance, accept mismatched MCP source bindings, omit advertised output schema, or accept non-object MCP roots, all fingerprint/environment/command/candidate/working-directory digests use independent fixed vectors, no test claims process groups are non-escapable containment or uses a released PGID, restricted sandbox inputs fail before host observation, repository-source and repository-target marker programs never execute, initial/checkpoint special-file opens cannot hang, retained working-directory `fchdir` and handle-relative candidate access survive cwd pathname replacement and match adapter current-dir semantics, zero-exit same-group children cannot yield success, owner reservation and setup failure occur before target exec, the existing detached `ManagedChild` reaper is not claimed as cancellation-complete evidence, the exact bounded `ETXTBSY` retry, active/cleanup deadlines, output-cap range, stable-key bound, MCP annotations and legacy dependencies remain behavior-sensitive, exact JSON resource-counting vectors are frozen, every cleanup/schema/resource-limit boundary is covered without weakened assertions, and current-head CI, Gemini review, and repository ruleset approval all pass; the original PR may close GH-1733 only after all gates pass. Verify: run every command and audit in Required Verification on one current implementation head, then collect fresh PR-gate evidence.
...
- [ ] Obtain mandatory human security review of Unix nonblocking target open,
      effective-sandbox fail-closed gating, retained working-directory open and
      `fchdir`, final-target repository-boundary authorization, the
      retained-handle pre-exec path, pre-reserved cleanup ownership,
      anchored process-group membership
...
- MCP server identity is derived from a typed exact stable configuration-entry
  ...
  reordered only at the eight approved schema-set keyword families;
  ...
  `dependentRequired` property arrays are string sets; legacy `dependencies`
  values are schemas or string sets; and fixed text/annotation/schema limits
  fail typed before unbounded work under the frozen root-depth, value-node,
  decoded-string, direct-entry, raw-byte, and canonical-byte counting rules.
  `contentSchema` always enters schema context, including nested `required` and
  `oneOf`.
