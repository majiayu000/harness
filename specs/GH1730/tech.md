...
| `configured_user` | A user-owned configured root with a persisted lowercase snake_case configuration key as the first relative segment |

For `configured_user`, the first segment after the root namespace is the
configuration key and must match `[a-z0-9]+(?:_[a-z0-9]+)*`. Validation
preserves the key exactly and rejects case variants, aliases, UUID shapes,
whitespace/display labels, and the reserved missing-evidence spellings before
validating the remaining relative portable path. It never normalizes an invalid
key into a valid one.
...
- [ ] Prove configured-user keys preserve valid lowercase snake_case and reject
      case variants, aliases, UUIDs, reserved sentinels, and display labels.
