...
- [ ] `SP1471-T003` Owner: `claude-adapters` | Dependencies: `SP1471-T001` | Done when: the Claude CodeAgent spawn path passes static instructions through `--append-system-prompt`, keeps `-p <dynamic prompt>` ordering, and enables `--exclude-dynamic-system-prompt-sections` for layered prompts | Verify: `cargo test -p harness-agents claude`
