...
- [ ] `SP1607-T002` Owner: `reducer` | Done when: `prompt_task_success_decision` replaces the fixed done arm in `reducer.rs` with branches for no-policy (unchanged), settled → done, active → continue with attempt-scoped `enqueue_activity` command and an atomic data patch for `attempt`, `last_external_state`, `last_summary = result.summary`, and `same_state_count`, malformed signal → blocked, exhaustion → blocked, no-progress → blocked; every branch attaches `external_state` evidence; a test reloads the committed instance and proves T005 can read the persisted previous summary without querying job history (B-004..B-009, B-011, B-013) | Verify: `cargo test -p harness-workflow reducer::prompt_continuation`
- [ ] `SP1607-T003` Owner: `validator` | Depends on: `SP1607-T001` | Covers: B-004 | Done when: `DecisionValidatorKind` has a `PromptTask` variant, `DecisionValidator::prompt_task()` uses it, and a dedicated prompt-task validator fail-closes every `implementing → implementing` decision unless its decision ID is exactly `continue_prompt_task` and it carries exactly one `EnqueueActivity` for `implement_prompt`; the allowlist no longer permits `Wait` on that self-loop; tests accept the valid continuation and reject an arbitrary decision ID, no command, `Wait` only, wrong activity, and multiple enqueue commands; continuation block decisions reuse the existing `allow_from_any("blocked", ...)` plus required `MarkBlocked` contract (GH-1603 alignment) | Verify: `cargo test -p harness-workflow prompt_task_validator`
...
The instance-data patch mechanism for counters and `last_summary` (atomic
with the state transition and next-attempt enqueue) is the one open
implementation detail — pin it during T002 against how existing reducers
persist decision side effects; the invariant is B-004/B-009/B-011
atomicity, not the mechanism. The signal shape
