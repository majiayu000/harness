...
def test_adoption_matrix_requires_name_and_repo(tmp_path: Path) -> None:
    write_text(tmp_path / "docs" / "ADOPTION_MATRIX.md", "matrix\n")
    write_text(tmp_path / "examples" / "rclean-smoke.md", "smoke\n")
    write_text(
        tmp_path / "examples" / "adoptions" / "matrix.json",
    ...
    assert any(
        check["id"] == "adoption_matrix.identity_present" and check["status"] == "fail"
        for check in checks
    )
    assert "rclean missing adoption name" in errors
