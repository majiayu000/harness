...
| `verify_merge_completion` | `true` | When true, Harness re-reads GitHub before accepting an agent-reported `merge_pr` success. When false, merge completion fails closed instead of trusting agent-reported state. |
