...
`WORKFLOW.md` under `storage`. Retention and the watchdog ship enabled by
default (GH-1879); the retention loops run their first passes in dry-run mode,
reporting what would be deleted, before destructive pruning engages:
...
The watchdog is enabled by default and read-only: aged `blocked` and
...
`runtime_retention_dry_run_passes` (default 1) controls how many passes after
process start report candidate terminal workflow families without deleting;
`0` disables dry-run. Retention deletes terminal workflow families older than
...
runtime events, and artifacts. Active workflow families are never pruned.

`task_retention_dry_run_passes` (default 1) is the task-retention equivalent.
